...
<?php
	...
	$where= "lab_id=$lbid $shopsql $datesql $statussql $invsql $refixsql";
	//echo $where;
	...
	$sql = "SELECT SUM(labcharges) AS jobs,SUM(paid) AS paid FROM repairs where lab_id=$lbid";
										...
										?>	
											<tr>
												...
												<?php $strid = $row['id'];
													  ...
													  $strfaults = json_encode($faults);
													  $strlchg = $row['labcharges'];
													  echo "<td data-title='Action'><a data-toggle='modal' href='#modal-signin' style='font-size:14px; font-weight:bold; color:#DE5050; text-decoration:underline;' onclick='setData($strid,$strmake,$strmodel,$strfaults,$strstatus,$strnotes,$strimei,$strlnotes,$strlchg);'><img src='images/add.png'></a></td>";
												?>												
...
<script type="text/javascript">
	function nextRecords() {
		...
		$.post("labrepairs_list.php",{pageNum:currentPage,txtshop:shop,txtsdate:sdate,txtedate:edate,txtstatus:status,txtinv:inv,txtrefix:refix},function(data){
	...
	function jumpToRecords(cpage) {
		...
		$.post("labrepairs_list.php",{pageNum:cpage,txtshop:shop,txtsdate:sdate,txtedate:edate,txtstatus:status,txtinv:inv,txtrefix:refix},function(data){
			$('#labrepairslist').html(data);
		});
	}
